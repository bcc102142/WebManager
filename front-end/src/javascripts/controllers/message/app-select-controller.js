...
import appSelectRoute from '@views/routes/app-select-route.html';
import {getHouseList} from '@models/message/message-model'
...
function renderItems(req,res) {
    return new Promise(async (resolve) => {
        let data = await getHouseList({ pageSize, pageNo, search,sortBy, sort: sort[sortBy]});
        // { pageSize, pageNo, search, sortBy, sort: sort[sortBy] }
       $('#web-content').html( 
            template.compile(appSelectRoute)({items:data.items})
            );
        pages = data.pages;
        resolve(data);
        sortHandler();
